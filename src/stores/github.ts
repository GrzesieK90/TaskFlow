...
export const useGithubStore = defineStore('github', {
  ...
  actions: {
    ...
    async fetchCommits(owner: string, repo: string) {
      this.loading = true
      this.error = null
      try {
        ...
        this.commits = data.map((item: any) => ({
          sha: item.sha,
    ...
    async fetchRepoStats(owner: string, repo: string) {
      this.loading = true
      this.error = null
      try {
    ...
    async fetchIssues(owner: string, repo: string) {
      ...
      try {
        ...
        const data = await response.json()
        this.issues = data.filter((item: any) => !item.pull_request).map((item: any) => ({
          id: item.id,
          number: item.number,
          title: item.title,
          state: item.state,
    ...
    async fetchPullRequests(owner: string, repo: string) {
      ...
      try {
        ...
        const data = await response.json()
        this.pullRequests = data.map((item: any) => ({
          id: item.id,
          number: item.number,
          title: item.title,
