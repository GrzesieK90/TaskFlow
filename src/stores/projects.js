...
export const useProjectStore = defineStore('projects', {
    ...
    getters: {
        ...
        highPriorityProjects: (state) => state.projects.filter(p => p.priority === 'high'),
        projectById: (state) => (id) => state.projects.find(p => p.id === id),
        // Nowe gettery dla filtrowania i sortowania
        getFilteredProjects: (state) => (searchQuery, status, priority, sortBy) => {
    ...
    actions: {
        ...
        saveToLocalStorage(description = 'Auto-save') {
            try {
